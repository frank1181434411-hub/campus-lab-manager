...
物理设计阶段主要将逻辑设计阶段形成的关系模式落实到具体的 MySQL 数据库中，并结合系统的业务规模，对存储结构、访问效率和数据备份方案进行设计。本系统属于高校机房管理课程设计，数据规模相对有限，因此物理设计以结构清晰、约束完整、查询稳定和便于部署为主要目标，不采用分布式数据库、水平分表和复杂分区等大型系统方案。

## 4.1 数据库存储结构设计

本系统采用 MySQL 作为数据库管理系统，表结构、约束、索引、视图和触发器统一定义在 `database/schema.sql` 文件中。

数据库字符集采用 `utf8mb4`，以保证学生姓名、课程名称、故障描述等中文信息能够正常存储和显示。数据库表采用支持事务、行级锁和外键约束的 InnoDB 存储引擎，以满足上机、下机、排课和报修等业务对数据一致性的要求。

系统中的数据表按照业务功能分为以下几类：

1. 用户及教学组织数据，包括 `user_account`、`student`、`teacher`、`admin`、`class_info` 和 `course`；
2. 机房资源数据，包括 `room` 和 `seat`；
3. 教学安排及上机数据，包括 `schedule` 和 `use_log`；
4. 设备维护数据，包括 `repair`。

各表均设置明确的主键。大部分实体使用单字段主键，机位表 `seat` 使用 `(room_id,seat_no)` 作为复合主键。这是因为机位号只在一个机房内部唯一，不同机房中可以存在相同的机位号。

表之间通过外键建立引用关系。例如，学生表通过 `class_id` 引用班级表，排课表通过教师编号、班级编号、课程编号和机房编号引用相应基础表，上机记录通过学生编号和机房号、机位号引用学生与机位，报修表通过用户编号和机位复合标识引用报修提交者、处理者和故障机位。

字段类型根据数据含义进行选择：

* 编号字段使用 `INT` 或 `VARCHAR`；
* 时间字段使用 `DATETIME`；
* 数量字段使用整数类型；
* 状态字段使用长度较短的字符串类型，并通过 `CHECK` 约束限制取值范围；
* 故障描述等较长文本使用较大的 `VARCHAR` 字段；
* 可选关联字段允许为 `NULL`，例如自由上机记录中的 `schedule_id` 和待处理报修中的 `handler_user_id`。

## 4.2 数据库性能优化设计

本系统的数据规模较小，但学生上机记录、排课记录和报修记录会随着使用时间不断增加，因此对高频查询字段建立了相应索引。

主要索引包括：

1. 学生表的班级编号索引，用于查询某班级中的学生；
2. 排课表的教师编号、班级编号及机房时间索引，用于查询教师课表、班级课表和机房占用情况；
3. 上机记录表的学生时间索引、机位时间索引和排课编号索引，用于查询学生上机历史、机位使用记录和课堂考勤记录；
4. 报修表的状态、提交用户和处理用户索引，用于查询待处理报修、用户提交的报修和管理员负责的报修。

对同一机房、同一学期、同一周次、同一星期和同一节次的排课设置唯一约束，以防止重复占用机房。登录账号、机位 IP 地址等需要保持唯一的数据也设置唯一约束。

系统建立了教师课表、学生上机历史、班级考勤、机房机位状态和报修详情等视图，用于封装常用的多表连接查询，减少应用程序中重复编写复杂 SQL 的情况。

Python 应用层通过参数化 SQL 查询数据库，避免直接拼接用户输入。新增排课、开始上机、结束上机、提交报修和更新维修状态等写操作使用数据库事务。当操作执行失败时，应用程序回滚事务，防止只完成部分更新。

对于可能返回大量数据的上机历史、报修记录和用户列表，应用层可使用分页查询，避免一次性加载全部数据。查询条件应尽量使用已经建立索引的字段，减少不必要的全表扫描。

由于本系统主要用于课程设计和小规模机房管理，暂不采用数据库分区、分库分表、主从复制和读写分离。若后续数据量明显增加，可考虑按照学期或年份对上机记录进行归档。

## 4.3 数据备份与恢复设计

数据库结构通过 `database/schema.sql` 文件保存，该文件可以重新创建数据库、数据表、索引、视图和触发器。测试数据通过 `database/sample_data.sql` 文件保存，用于快速建立课程设计演示环境。
...
正式运行过程中产生的学生上机记录、考勤记录和报修记录不能只依赖样例数据文件恢复，应定期使用 `mysqldump` 导出数据库。

备份命令示例如下：

```bash
mysqldump -u root -p --routines --triggers campus_lab_manager > campus_lab_manager_backup.sql
```

恢复命令示例如下：

```bash
mysql -u root -p campus_lab_manager < campus_lab_manager_backup.sql
```

备份文件应按照日期命名，并保存在与运行数据库不同的位置。进行数据库结构重建或批量修改前，应先备份现有数据。

由于 `schema.sql` 中包含删除旧表和重新建表的语句，因此该文件主要用于首次部署和重置测试环境，不应在保存真实业务数据的数据库中随意重复执行。

数据库账号密码、Flask 密钥和实际运行环境配置通过 `.env` 文件保存，`.env` 文件应加入 `.gitignore`，不能上传到公开代码仓库。正式数据库备份中可能包含用户账号和业务数据，也不应直接上传到 GitHub。
...
本系统在数据库设计的基础上，实现了一个简易 Web 应用，用于完成学生、教师和管理员三类用户的核心业务操作。应用系统以数据库课程设计演示为目标，重点展示登录认证、角色权限、机房机位管理、排课管理、上机记录、课堂考勤和设备报修等功能。
...
系统使用原生 SQL 访问数据库，不使用 ORM 框架。数据库连接信息通过 `.env` 文件配置，避免在代码中直接保存数据库密码。

## 5.2 系统功能实现

系统按照用户角色划分为学生端、教师端和管理员端。

学生端主要实现个人信息查看、机房机位查询、自由上机、课堂上机、下机、个人上机历史查询、个人考勤查询和设备报修功能。

教师端主要实现教师课表查询、课堂考勤名单查看、机房状态查询和设备报修功能。

管理员端主要实现用户账号状态管理、学生与教师基础信息查看、机房和机位维护、排课新增修改删除、上机记录查看、机房利用统计和报修处理功能。

系统通过 Flask session 保存当前登录用户信息，并根据 `user_account.role` 区分 `student`、`teacher` 和 `admin` 三种角色。不同角色只能访问对应权限范围内的页面。
...
| 文件或目录 | 作用 |
|---|---|
| `app/main.py` | Flask 路由、登录控制、角色权限控制 |
| `app/config.py` | 读取环境变量和考勤节次配置 |
| `app/db.py` | 数据库连接、查询、事务和异常处理 |
| `app/services/auth_service.py` | 用户登录、账号状态、密码修改 |
| `app/services/room_service.py` | 机房与机位查询和维护 |
| `app/services/use_log_service.py` | 上机、下机、考勤和利用率统计 |
| `app/services/schedule_service.py` | 排课查询、新增、修改和删除 |
| `app/services/repair_service.py` | 报修提交、分配和状态更新 |
| `app/templates/` | 页面模板 |
| `app/static/style.css` | 页面样式 |

数据库写操作均通过事务执行。若插入或更新过程中触发 MySQL 约束或触发器错误，系统会回滚事务，并在页面上显示中文错误提示。

## 5.4 系统测试说明

系统测试主要包括数据库测试和 Web 功能测试两部分。

数据库测试方面，可通过 `database/queries.sql` 检查用户账号、学生班级、教师课表、空闲机位、学生上机历史、课堂考勤、待处理报修、故障机位、出勤率和机房使用率等查询。

Web 功能测试方面，使用 `sample_data.sql` 中的初始账号进行演示：
...
测试时应重点检查登录权限、自由上机、课堂上机、下机释放机位、报修后锁定机位、维修完成后恢复机位、排课冲突限制和不同角色访问权限。

---

# 六、总结分析

本系统围绕高校机房管理场景，完成了从需求分析、概念设计、逻辑设计到数据库和 Web 应用实现的基本流程。系统能够较完整地展示关系数据库在信息管理系统中的应用方式。

## 6.1 设计过程中遇到的问题

设计过程中主要遇到以下问题：

1. 用户账号、学生、教师和管理员之间既有统一登录需求，又有不同角色扩展信息，需要合理拆分表结构；
2. 机位状态会随上机、下机、报修和维修完成发生变化，仅靠普通外键难以表达；
3. 排课时间使用学期、周次、星期和节次表示，而考勤记录使用具体日期时间，二者之间存在转换问题；
4. 报修提交人和处理人的角色约束无法仅通过普通外键保证；
5. Web 应用需要同时考虑数据库约束、业务提示和页面权限控制。

## 6.2 问题解决方法

针对上述问题，本系统采用了以下解决方法：

1. 使用 `user_account` 统一保存登录账号，`student`、`teacher`、`admin` 保存角色扩展信息；
2. 使用触发器实现机位状态联动，例如上机后占用机位、下机后释放机位、报修后锁定机位、维修完成后恢复机位；
3. 使用排课表中的 `class_period` 配合应用层配置 `CLASS_PERIODS` 进行考勤状态判断；
4. 使用触发器检查报修提交人和处理人的角色是否合法；
5. Web 应用中使用 session 和角色检查装饰器限制不同用户访问范围。

## 6.3 系统不足与改进方向

当前系统仍存在一些不足：

1. 排课没有保存具体日期，考勤计算只能基于当前日期和节次配置进行近似判断；
2. 前端页面以课程设计演示为主，交互体验还可以进一步完善；
3. 用户管理目前只实现了账号状态修改，尚未提供完整的新增、编辑和删除页面；
4. 系统未实现批量导入学生、课程和排课数据；
5. 密码策略和安全控制较简单，正式系统应增加更完善的认证和日志机制。

后续可以增加校历表、节次时间表、批量导入、操作日志、数据导出和更完整的后台管理功能。
...
[2] 王珊, 张俊, 卢卫. 数据库系统概论（第6版）习题解析与实验指导[M]. 北京: 高等教育出版社, 2024.

[3] Oracle Corporation. MySQL 8.0 Reference Manual[EB/OL]. [2026-06-24]. https://dev.mysql.com/doc/refman/8.0/en/.

[4] Pallets Projects. Flask Documentation[EB/OL]. [2026-06-24]. https://flask.palletsprojects.com/.

[5] PyMySQL Contributors. PyMySQL Documentation[EB/OL]. [2026-06-24]. https://pymysql.readthedocs.io/.

[6] Python Software Foundation. Python 3.12 Documentation[EB/OL]. [2026-06-24]. https://docs.python.org/3.12/.

[7] 高校机房管理系统源代码[EB/OL]. 2026[2026-06-24]. https://github.com/frank1181434411-hub/campus-lab-manager.
...
本应用采用 Python、Flask、PyMySQL、python-dotenv、MySQL、Jinja2、HTML 和 CSS 实现。数据库访问使用原生 SQL 和参数化查询，不使用 SQLAlchemy、Django 或前端框架。
...
先确保 MySQL 服务已启动，然后导入数据库结构和演示数据：

```bash
mysql --default-character-set=utf8mb4 -u root -p < database/schema.sql
mysql --default-character-set=utf8mb4 -u root -p < database/sample_data.sql
```

`schema.sql` 会创建数据库 `campus_lab_manager`，并创建表、索引、视图和触发器。`sample_data.sql` 会导入课程设计演示所需的基础账号、班级、学生、教师、机房、机位、排课和报修数据。

## 7.5 环境变量配置

复制 `.env.example` 为 `.env`，再根据本机数据库信息修改：
...
DB_USER=root
DB_PASSWORD=
DB_NAME=campus_lab_manager
FLASK_HOST=127.0.0.1
FLASK_PORT=5000
FLASK_DEBUG=true
ATTENDANCE_LATE_MINUTES=10
ATTENDANCE_EARLY_LEAVE_MINUTES=10
```

不要把真实数据库密码提交到仓库。`.env` 已由 `.gitignore` 忽略。
...
说明：这些明文密码仅用于课程演示和初始数据导入。应用中新创建或修改的密码会使用 Werkzeug 哈希保存，正式系统中不应保存明文密码。

## 7.9 功能演示步骤

1. 使用管理员账号 `admin001` 登录；
2. 查看管理员首页统计信息；
3. 进入“机房机位”页面，查看 R101、R102 的机位状态；
4. 进入“排课管理”页面，查看示例排课并尝试新增排课；
5. 进入“设备报修”页面，分配待处理报修并更新维修状态；
6. 退出后使用学生账号 `stu001` 登录；
7. 查看学生首页中的班级排课和机房状态；
8. 输入机房号和机位号开始自由上机；
9. 下机后查看个人上机历史；
10. 提交设备报修并查看报修状态；
11. 退出后使用教师账号 `t001` 登录；
12. 查看本人课表，进入某节课查看考勤名单。

## 7.10 已知限制

1. 当前排课表使用“学期、周次、星期、节次”表示课程时间，没有保存具体上课日期；
2. 课堂考勤的迟到和早退判断使用当前日期加 `CLASS_PERIODS` 中的节次时间进行近似计算；
3. 缺勤名单通过排课班级学生与课堂上机记录左连接动态生成，不强制插入虚假的缺勤 `use_log`；
4. 前端页面以课程设计演示为主，没有实现复杂后台框架；
5. 示例数据中的初始密码为明文，仅用于演示兼容，后续修改密码会保存为哈希。
