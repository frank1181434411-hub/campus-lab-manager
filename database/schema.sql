...
CREATE TABLE user_account (
    user_id INT PRIMARY KEY AUTO_INCREMENT,username VARCHAR(50) NOT NULL UNIQUE,password VARCHAR(100) NOT NULL,role VARCHAR(20) NOT NULL,status VARCHAR(20) NOT NULL DEFAULT 'active',phone VARCHAR(20),CHECK (role IN ('student','teacher','admin')),CHECK (status IN ('active','disabled','locked'))
) ENGINE=InnoDB;

CREATE TABLE class_info (
    class_id VARCHAR(20) PRIMARY KEY,class_name VARCHAR(50) NOT NULL,major VARCHAR(50) NOT NULL,total_number INT NOT NULL DEFAULT 0,CHECK (total_number >=0)
) ENGINE=InnoDB;

CREATE TABLE student (
    student_id VARCHAR(20) PRIMARY KEY,user_id INT NOT NULL UNIQUE,student_name VARCHAR(50) NOT NULL,gender VARCHAR(10),grade VARCHAR(20),class_id VARCHAR(20) NOT NULL,FOREIGN KEY (user_id) REFERENCES user_account(user_id),FOREIGN KEY (class_id) REFERENCES class_info(class_id)
) ENGINE=InnoDB;

CREATE TABLE teacher (
    teacher_id VARCHAR(20) PRIMARY KEY,user_id INT NOT NULL UNIQUE,teacher_name VARCHAR(50) NOT NULL,title VARCHAR(50),FOREIGN KEY (user_id) REFERENCES user_account(user_id)
) ENGINE=InnoDB;

CREATE TABLE admin (
    admin_id VARCHAR(20) PRIMARY KEY,user_id INT NOT NULL UNIQUE,admin_name VARCHAR(50) NOT NULL,FOREIGN KEY (user_id) REFERENCES user_account(user_id)
) ENGINE=InnoDB;

CREATE TABLE course (
    course_id VARCHAR(20) PRIMARY KEY,course_name VARCHAR(50) NOT NULL,total_hours INT NOT NULL,course_type VARCHAR(20),CHECK (total_hours > 0)
) ENGINE=InnoDB;

CREATE TABLE room (
    room_id VARCHAR(20) PRIMARY KEY,room_location VARCHAR(100) NOT NULL,total_seats INT NOT NULL,open_status VARCHAR(20) NOT NULL DEFAULT 'open',CHECK (total_seats >=0),CHECK (open_status IN ('open','closed','maintenance'))
) ENGINE=InnoDB;

CREATE TABLE seat (
    room_id VARCHAR(20) NOT NULL,seat_no VARCHAR(20) NOT NULL,ip_address VARCHAR(50) UNIQUE,machine_config VARCHAR(200),seat_status VARCHAR(20) NOT NULL DEFAULT 'free',PRIMARY KEY (room_id,seat_no),FOREIGN KEY (room_id) REFERENCES room(room_id),CHECK (seat_status IN ('free','self_study','class_in_use','fault'))
) ENGINE=InnoDB;

CREATE TABLE schedule (
    schedule_id INT PRIMARY KEY AUTO_INCREMENT,semester VARCHAR(20) NOT NULL,week_no VARCHAR(50) NOT NULL,weekday TINYINT NOT NULL,class_period VARCHAR(20) NOT NULL,teacher_id VARCHAR(20) NOT NULL,class_id VARCHAR(20) NOT NULL,course_id VARCHAR(20) NOT NULL,room_id VARCHAR(20) NOT NULL,FOREIGN KEY (teacher_id) REFERENCES teacher(teacher_id),FOREIGN KEY (class_id) REFERENCES class_info(class_id),FOREIGN KEY (course_id) REFERENCES course(course_id),FOREIGN KEY (room_id) REFERENCES room(room_id),UNIQUE (room_id,semester,week_no,weekday,class_period),CHECK (weekday BETWEEN 1 AND 7)
) ENGINE=InnoDB;

CREATE TABLE use_log (
    log_id INT PRIMARY KEY AUTO_INCREMENT,student_id VARCHAR(20) NOT NULL,room_id VARCHAR(20) NOT NULL,seat_no VARCHAR(20) NOT NULL,schedule_id INT NULL,start_time DATETIME NOT NULL,end_time DATETIME NULL,use_type VARCHAR(20) NOT NULL,attendance_status VARCHAR(20) NOT NULL DEFAULT 'not_applicable',FOREIGN KEY (student_id) REFERENCES student(student_id),FOREIGN KEY (room_id,seat_no) REFERENCES seat(room_id,seat_no),FOREIGN KEY (schedule_id) REFERENCES schedule(schedule_id),CHECK (use_type IN ('free','class')),CHECK (attendance_status IN ('normal','late','early_leave','absent','not_applicable')),CHECK (end_time IS NULL OR end_time >=start_time),CHECK (
...
CREATE TABLE repair (
    repair_id INT PRIMARY KEY AUTO_INCREMENT,submitter_user_id INT NOT NULL,handler_user_id INT NULL,room_id VARCHAR(20) NOT NULL,seat_no VARCHAR(20) NOT NULL,report_time DATETIME NOT NULL,fault_description VARCHAR(500) NOT NULL,repair_status VARCHAR(20) NOT NULL DEFAULT 'pending',FOREIGN KEY (submitter_user_id) REFERENCES user_account(user_id),FOREIGN KEY (handler_user_id) REFERENCES user_account(user_id),FOREIGN KEY (room_id,seat_no) REFERENCES seat(room_id,seat_no),CHECK (repair_status IN ('pending','processing','done'))
) ENGINE=InnoDB;
...
SELECT
    s.student_id,s.student_name,r.room_id,r.room_location,ul.seat_no,ul.start_time,ul.end_time,ul.use_type,ul.attendance_status
FROM use_log ul
...
SELECT
    t.teacher_id,t.teacher_name,sc.schedule_id,c.course_name,ci.class_name,r.room_id,r.room_location,sc.semester,sc.week_no,sc.weekday,sc.class_period
FROM schedule sc
...
SELECT
    sc.schedule_id,c.course_name,ci.class_name,t.teacher_name,s.student_id,s.student_name,ul.start_time,ul.end_time,ul.attendance_status
FROM schedule sc
JOIN course c ON sc.course_id=c.course_id
JOIN class_info ci ON sc.class_id=ci.class_id
JOIN teacher t ON sc.teacher_id=t.teacher_id
...
SELECT
    r.room_id,r.room_location,r.open_status,se.seat_no,se.ip_address,se.machine_config,se.seat_status
FROM room r
JOIN seat se ON r.room_id=se.room_id;

CREATE VIEW v_repair_detail AS
SELECT
    rp.repair_id,rp.room_id,rp.seat_no,rp.report_time,rp.fault_description,rp.repair_status,submitter.username AS submitter_username,submitter.role AS submitter_role,handler.username AS handler_username,handler.role AS handler_role
FROM repair rp
...
BEGIN
    ...
    SELECT open_status INTO room_status
    FROM room
...
BEGIN
    ...
    SELECT se.seat_status,r.open_status
    INTO current_seat_status,current_room_status
    FROM seat se
    ...
    SELECT COUNT(*) INTO active_log_count
    FROM use_log
    WHERE room_id=NEW.room_id
    ...
    ELSE
        ...
        INTO schedule_room_id,schedule_class_id
        FROM schedule
