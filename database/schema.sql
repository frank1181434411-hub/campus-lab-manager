...
CREATE TABLE use_log (
    ...
    FOREIGN KEY (room_id,
    seat_no) REFERENCES seat(room_id,
    seat_no),
    FOREIGN KEY (schedule_id) REFERENCES schedule(schedule_id),
    CHECK (use_type IN ('free',
    'class')),
    CHECK (attendance_status IN ('normal',
    'late',
    'early_leave',
    'absent',
    'not_applicable')),
...
CREATE TABLE repair (
    ...
    seat_no) REFERENCES seat(room_id,
    seat_no),
    CHECK (repair_status IN ('pending',
    'processing',
    'done'))
) ENGINE=InnoDB;
...
BEGIN
    ...
    SELECT se.seat_status,
    r.open_status
    INTO current_seat_status,
    current_room_status
    FROM seat se
